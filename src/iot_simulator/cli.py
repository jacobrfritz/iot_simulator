...
def parse_distribution(value):
    try:
        ...
        raise argparse.ArgumentTypeError("Distributions must be formatted as 'type,clients' (e.g., normal,10)")
...
def parse_args(args: list[str]) -> argparse.Namespace:
    ...
    for config in parsed_args.dist:
        print(f"Spawning {config['clients']} clients with {config['type']} distribution.")
